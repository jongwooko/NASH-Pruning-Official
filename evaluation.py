...
from models.deploying_t5 import NashT5ForConditionalGeneration
...
if __name__ == '__main__':
    ...
    # load model
    model_class = NashT5ForConditionalGeneration
    ...
    if early_exit_layer is not None:
        model.config.decoder_early_exit = True
        model.config.early_exit_layer = early_exit_layer

        model.decoder.config.decoder_early_exit = True
        model.decoder.config.early_exit_layer = early_exit_layer
    ...
    for i in range(len(model.decoder.block)):
        total_block_times += model.decoder.block[i].block_times
    enc_block_times = 0.0
    for i in range(len(model.encoder.block)):
        total_block_times += model.encoder.block[i].block_times
