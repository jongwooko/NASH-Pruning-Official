...
class BartAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = nn.Linear(embed_dim, self.inner_dim, bias=bias) # modified --> (original) embed_dim => embed_dim
    ...
    def forward(
        ...
        attn_output = attn_output.reshape(bsz, tgt_len, self.inner_dim) # originally self.embed_dim
...
class BartEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_start_time = None
        feed_forward_end_time = None
        
        residual = hidden_states
        if self.self_attn.v_proj == None:
            attn_weights = None
            pass
        else:
            ...
            if head_layer_z is not None: ## apply attention layer mask to the attention outputs
                hidden_states = hidden_states.mul(head_layer_z)
            hidden_states = residual + hidden_states
            hidden_states = self.self_attn_layer_norm(hidden_states) # layer norm
        ...
        if self.fc1 == None:
            ...
            ## copied from 
            if hidden_states is None: # maybe works.
                outputs = (residual,)
                if output_attentions:
                    outputs += (attn_weights,)
                return outputs
        ...
        if self_attention_start_time is not None:
            self.self_attention_times += (self_attention_end_time - self_attention_start_time)
        if feed_forward_end_time is not None:
...
class BartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        block_start_time = time.time()
        self_attention_end_time = None
        cross_attention_end_time = None
        feed_forward_end_time = None
        ...
        if self.self_attn.v_proj == None:
            ...
            if dec_self_head_layer_z is not None:
                hidden_states *= dec_self_head_layer_z
            hidden_states = residual + hidden_states
        ...
        if encoder_hidden_states is not None:
            ...
            if self.encoder_attn.v_proj == None:
                cross_attn_weights = None
                cross_ttn_present_key_value = (None,)
            else:
                ...
                hidden_states, cross_attn_weights, cross_attn_present_key_value = self.encoder_attn(
                    ...
                    output_attentions=output_attentions,
                    head_z=dec_cross_head_z,
                )
                hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
                if dec_cross_head_layer_z is not None:
                    hidden_states *= dec_cross_head_layer_z
                hidden_states = residual + hidden_states
        ...
        if self.fc1 == None:
            ...
            feed_forward_start_time = time.time()
            # Fully Connected # 여기는 더 추가해야함!
            residual = hidden_states
            hidden_states = self.activation_fn(self.fc1(hidden_states))
            hidden_states = nn.functional.dropout(hidden_states, p=self.activation_dropout, training=self.training)
            if dec_intermediate_z is not None:
                hidden_states = hidden_states.mul(dec_intermediate_z)
            hidden_states = self.fc2(hidden_states)
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        ...
        block_end_time = time.time()
        if self_attention_end_time is not None:
            self.self_attention_times += (self_attention_end_time - self_attention_start_time)
        ...
        self.block_times_list = [block_start_time, block_end_time]
        if cross_attention_end_time is not None:
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    dec_self_head_z = dec_self_head_z[idx] if dec_self_head_z is not None else None,
                    dec_self_head_layer_z = dec_self_head_layer_z[idx] if dec_self_head_layer_z is not None else None,
                    dec_cross_head_z = dec_cross_head_z[idx] if dec_cross_head_z is not None else None,
                    dec_cross_head_layer_z = dec_cross_head_layer_z[idx] if dec_cross_head_layer_z is not None else None,
                    dec_mlp_z = dec_mlp_z[idx] if dec_mlp_z is not None else None,
                    dec_intermediate_z = dec_intermediate_z[idx] if dec_intermediate_z is not None else None,
                    inference=False,
                )
            hidden_states = layer_outputs[0]
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        self.enc_times += self.model.enc_times
        self.dec_times += self.model.dec_times
        self.head_times += (head_end_time - head_start_time) 
        self.total_times += self.model.total_times + self.head_times
...
class BartForCausalLM(BartPretrainedModel):
    ...
    def forward(
        ...
        loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss(ignore_index=-100) # edited
