...
class NashT5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.self_attention_times = 0.0
        self.cross_attention_times = 0.0
        self.feed_forward_times = 0.0
        self.block_times = 0.0
    ...
    def forward(
        ...
        self.self_attention_times += (self_attention_end_time - self_attention_start_time)
        self.feed_forward_times += (feed_forward_end_time - feed_forward_start_time)
        self.block_times += (block_end_time - block_start_time)
        
        self.block_times_list = [block_start_time, block_end_time]
        
        if self.is_decoder:
            self.cross_attention_times += (cross_attention_end_time - cross_attention_start_time)
...
class NashT5LayerFF(nn.Module):
    ...
    def forward(self, hidden_states, intermediate_z=None, mlp_z=None, hidden_z=None, inference=False):
        if self.DenseReluDense.wo is not None:
...
class NashT5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.attention_times_1 = 0.0
        self.attention_times_2 = 0.0
        self.attention_times_3 = 0.0
        self.attention_times_4 = 0.0
    ...
    def forward(
        ...
        if self.v is None:
            return (None, None, position_bias, None) if output_attentions else (None, None, position_bias)
        ...
        qkv_start_time = all_start_time = time.time()
        # get query states
        ...
        att_mid2_time = time.time()
        
        if heads_idx is not None:
            revised_position_bias = position_bias[:, heads_idx]
            revised_position_bias = revised_position_bias.to(scores.device)
        else:
            revised_position_bias = position_bias
        att_mid3_time = time.time()
        ...
        self.qkv_times += (qkv_end_time - qkv_start_time)
        self.att_times += (att_end_time - att_start_time)
        self.proj_times += (proj_end_time - proj_start_time)
        self.all_times += (all_end_time - all_start_time)
        
        self.attention_times_1 += (att_mid1_time - att_start_time)
        self.attention_times_2 += (att_mid2_time - att_mid1_time)
        self.attention_times_3 += (att_mid3_time - att_mid2_time)
        self.attention_times_4 += (att_end_time - att_mid3_time)
        return outputs
...
class NashT5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.layer_norm_times = 0.0
        self.attention_times = 0.0
        self.dropout_times = 0.0
    ...
    def forward(
        ...
        if self.SelfAttention.q is not None:
            ...
            layer_norm_start_time = layer_norm_end_time = time.time()
        ...
        if self.SelfAttention.q is not None:
            ...
            dropout_start_time = dropout_end_time = time.time()
        
        outputs = (hidden_states,) + attention_output[1:]  # add attentions if we output them
        
        self.layer_norm_times += (layer_norm_end_time - layer_norm_start_time)
        self.attention_times += (attention_end_time - attention_start_time)
        self.dropout_times += (dropout_end_time - dropout_start_time)
...
class NashT5LayerCrossAttention(nn.Module):
    ...
    def forward(
        ...
        if self.EncDecAttention.q is not None:
            layer_norm_start_time = time.time()
            normed_hidden_states = self.layer_norm(hidden_states, hidden_z)
            layer_norm_end_time = time.time()
        else:
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if hasattr(self.config, "enc_pruned_heads") and not self.is_decoder:
                if len(self.enc_self_idx) < self.config.num_layers:
                    try:
                    ...
                    if len(self_index) == self.config.num_heads:
                        self_index = None

                    else:
                        self_index = torch.tensor(self_index).cuda()
                        
                    self.enc_self_idx.append(self_index)
                
                else:
                    self_index = self.enc_self_idx[i]
                
            elif hasattr(self.config, "dec_self_pruned_heads") and self.is_decoder:
                if len(self.dec_self_idx) < self.config.num_layers:
                    try:
                    ...
                    self.dec_self_idx.append(self_index)
                        
                else:
                    self_index = self.dec_self_idx[i]
                
            else:
                self_index = None
            
            if self.is_decoder and hasattr(self.config, "dec_cross_pruned_heads"):
                if len(self.dec_cross_idx) < self.config.num_layers:
                    try:
                    ...
                    if len(cross_index) == self.config.num_heads:
                        cross_index = None

                    else: 
                        cross_index = torch.tensor(cross_index).cuda()
                        
                    self.dec_cross_idx.append(cross_index)
                        
                else:
                    cross_index = self.dec_cross_idx[i]
...
class NashT5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        self.head_times += (head_end_time - head_start_time)
        self.enc_times += (enc_end_time - enc_start_time)
        self.dec_times += (dec_end_time - dec_start_time)
        self.total_times += (total_end_time - total_start_time)
